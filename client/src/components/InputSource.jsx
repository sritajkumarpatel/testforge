...
export default function InputSource({ config, onInputReady, providerName, pipelineStage, onSwitchToSettings, agentMode = 'regular', onAgentModeChange }) {
  ...
  const fetchAdo = async () => {
    ...
    if (!adoId.trim()) { setStatus('Enter a work item ID.'); setStatusClass('err'); return; }
    setLoading(true);
    ...
    <div className="card card-emphasis" id="adoLoadCard">
        ...
        <div className="ai-tabs">
          {['file', 'ado', 'paste'].map((t) => (
            <button key={t} className={`ai-tab${tab === t ? ' active' : ''}`} onClick={() => setTab(t)}>
              <span className="material-icons">{t === 'file' ? 'upload_file' : t === 'ado' ? 'cloud' : 'edit_note'}</span>
      ...
      <div className="ado-card-body">
        <p className="ado-hint">
          Load requirements from a document, fetch an ADO work item, or paste text directly. The three AI agents will analyze and generate test cases.
        </p>

        {tab === 'file' && (
          <>
            <div className="drop-zone" onClick={() => fileRef.current?.click()} onDragOver={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = '#00695c'; e.currentTarget.style.background = '#e0f2f1'; }} onDragLeave={(e) => { e.currentTarget.style.borderColor = '#b2dfdb'; e.currentTarget.style.background = '#f4fffe'; }} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer?.files?.[0]; if (f) loadFile(f); }}>
              <span className="material-icons">upload_file</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13.5, color: '#00695c', fontWeight: 600 }}>
                  {fileName || 'Drop a file here or click to browse'}
                </div>
                <div className="drop-zone-hint">Supports PDF, Word (.docx), Markdown, text, code files</div>
              </div>
            </div>
            <input ref={fileRef} type="file" accept={FILE_TYPES} style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) loadFile(f); }} />
          </>
        )}
          ...
          <>
            {org && project ? (
              <div className="settings-section-grid" style={{ marginBottom: 14 }}>
                ...
                <div className="ado-field">
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input value={adoId} onChange={(e) => setAdoId(e.target.value)} placeholder="e.g. 12345" />
                    <button className="btn btn-outline btn-sm" onClick={fetchAdo} disabled={loading}>
              ...
              <div className="ado-missing-config">
                ...
                <div>
                  <p style={{ margin: '0 0 4px', fontWeight: 600, color: '#111827', fontSize: 14 }}>Azure DevOps not configured</p>
                  <p style={{ margin: 0, color: '#6b7280', fontSize: 13 }}>Set your Organisation and Project in <a href="#" onClick={(e) => { e.preventDefault(); onSwitchToSettings(); }}>Settings</a> to fetch work items.</p>
                </div>
              </div>
            )}
          </>
        ...
        {tab === 'paste' && (
          <div className="ado-field" style={{ marginBottom: 14 }}>
            ...
            <textarea className="mono-textarea" value={rawText} onChange={(e) => setRawText(e.target.value)} placeholder="Paste user story, feature specification, or any requirements text here…" style={{ minHeight: 200 }} />
          </div>
        )}
        ...
        {status && (
          <div className="ado-parse-row" style={{ marginTop: 8, paddingTop: 8 }}>
            <span className={`ado-parse-status${statusClass ? ` ${statusClass}` : ''}`}>{status}</span>
          </div>
        )}
      </div>
    </div>
