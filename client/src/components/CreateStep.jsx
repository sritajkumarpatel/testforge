export default function CreateStep({ config, parsedScenarios, scenarioCount, chromeStatus, setChromeStatus, isRunning, setIsRunning, setLogs, setResultRows, setSummary, setPipelineStage }) {
  const launchChrome = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setChromeStatus({ text: `✓ Chrome launched — open ADO. Sign in, then continue below.`, cls: 'ok', url: data.url });
      } else {
  ...
  const startRun = async () => {
    ...
    if (!parsedScenarios.length) { alert('Parse your scenarios first.'); return; }
    if (!confirm(`Create ${scenarioCount} test case${scenarioCount !== 1 ? 's' : ''} in ${config.org}/${config.project}?`)) return;
    ...
    } catch (err) { appendLog(`Network error: ${err.message}`, 'error'); setIsRunning(false); return; }
    if (!response.ok && !response.body) { appendLog(`Server error ${response.status}`, 'error'); setIsRunning(false); return; }
    ...
    } catch (err) { appendLog(`Stream error: ${err.message}`, 'error'); }
    setIsRunning(false);
  };
  ...
  const handleEvent = (ev) => {
    switch (ev.type) {
      ...
      case 'error': appendLog(`✗ ${ev.message}`, 'error'); break;
      case 'status': appendLog(`${ev.total} queued`, 'info'); break;
      case 'case-done': {
        setResultRows((prev) => { const r = [...prev]; r[ev.index] = ev; return r; });
      ...
      case 'done': {
        appendLog(`Done — Created: ${ev.totalCreated} Failed: ${ev.totalFailed}`, ev.totalFailed > 0 ? 'error' : 'success');
        setSummary({ created: ev.totalCreated, failed: ev.totalFailed, total: ev.totalCreated + ev.totalFailed });
        setPipelineStage('results');
        break;
      }
      default: console.warn('unknown event', ev.type, ev);
    }
  };
    ...
    <div className="card card-emphasis" id="adoCreateCard">
      ...
      <div className="ado-card-body">
        ...
        <div className="ado-step-row">
          ...
          <span className="ado-step-desc">Creates all parsed test cases as ADO work items using your browser session.</span>
        </div>
      </div>
    </div>
  );
